...
import { Plus, Search, Filter, Trash2, Edit, User, CreditCard, Building2, Phone, Home, FileUp, FileDown, AlertCircle, Loader2, Database, Users } from "lucide-react";
        ...
        <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-blue-900 p-8 rounded-3xl text-white shadow-2xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold tracking-tight">Student Information System / વિદ્યાર્થી માહિતી</h1>
                <div className="flex items-center gap-3 mt-2">
                  {!firestore ? (
                    <Badge variant="destructive" className="bg-red-500/20 border-red-500/50 text-red-200">
                  ...
                  <span className="text-indigo-200 text-xs font-bold uppercase tracking-widest">Session: {academicYear}</span>
                </div>
              </div>
            </div>
            
            <div className="flex items-center gap-2">
              <Button variant="outline" className="bg-white/10 border-white/20 text-white hover:bg-white/20 font-bold hidden md:flex" asChild>
                <Link href="/bulk-entry">
                  <FileUp className="w-4 h-4 mr-2" />
                  Import / આયાત
                </Link>
              </Button>
              <Button variant="outline" className="bg-white/10 border-white/20 text-white hover:bg-white/20 font-bold hidden md:flex" onClick={exportToCSV}>
              ...
              <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
                ...
                <DialogTrigger asChild>
                  <Button className="bg-white text-indigo-900 hover:bg-indigo-50 font-black shadow-lg">
                    <Plus className="w-4 h-4 mr-2" />
                    Enroll Student / ઉમેરો
                  </Button>
                </DialogTrigger>
                {/* Dialog Content Omitted for Brevity - keeping previous functionality */}
          ...
          <ScrollArea className="w-full">
            <Table>
              <TableHeader className="bg-slate-50/50">
                <TableRow>
                  <TableHead className="font-black uppercase tracking-wider text-[10px] whitespace-nowrap text-center">Roll</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">G.R. No</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Full Name / નામ</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Gender</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Standard</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Birthday</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap text-center">Att.</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Caste</TableHead>
                  <TableHead className="font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Mobile</TableHead>
                  <TableHead className="text-right font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {studentsLoaded ? (
                  filteredStudents.length > 0 ? (
                    filteredStudents.map((s) => (
                      <TableRow key={s.id} className="hover:bg-indigo-50/30 transition-colors">
                        <TableCell className="font-black text-indigo-700 whitespace-nowrap text-center">{s.rollNumber}</TableCell>
                        <TableCell className="font-bold text-slate-400 whitespace-nowrap">{s.grNumber}</TableCell>
                        <TableCell className="font-bold text-slate-800 whitespace-nowrap">{s.name}</TableCell>
                        <TableCell className="whitespace-nowrap">
                          <Badge variant="outline" className="text-[10px] uppercase border-indigo-100 text-indigo-600 bg-indigo-50/50">
                            {s.gender === 'Male' ? 'Male' : s.gender === 'Female' ? 'Female' : 'Other'}
                          </Badge>
                        </TableCell>
                        <TableCell className="whitespace-nowrap">
                          <Badge variant="secondary" className="font-bold bg-blue-50 text-blue-700 border-blue-100">{s.academicStandard}</Badge>
                        </TableCell>
                        <TableCell className="whitespace-nowrap font-medium text-slate-600">{s.birthday}</TableCell>
                        <TableCell className="whitespace-nowrap text-center">
                          <span className={`font-black ${(s.attendance || 0) < 75 ? 'text-rose-500' : 'text-emerald-600'}`}>
                            {s.attendance || 0}%
                          </span>
                        </TableCell>
                        <TableCell className="whitespace-nowrap text-slate-600">{s.caste}</TableCell>
                        <TableCell className="whitespace-nowrap text-xs text-slate-500">{s.mobileNumber}</TableCell>
                        <TableCell className="text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" onClick={() => handleEdit(s)} className="hover:text-indigo-600 hover:bg-indigo-50">
                              <Edit className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => deleteStudent(s.id)} className="hover:text-rose-600 hover:bg-rose-50">
                  ...
                  <TableRow>
                    <TableCell colSpan={10} className="h-32 text-center">
                      <div className="flex flex-col items-center gap-2 text-muted-foreground">
                        ...
                        <p className="text-xs font-bold uppercase tracking-widest">Syncing student database...</p>
                      </div>
                    </TableCell>
                  </TableRow>
                )}
